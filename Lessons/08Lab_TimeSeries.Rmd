...
sum(is.na(BR_complete$Temperature_C)) #None
sum(is.na(BR_complete$Rainfall)) #None
...
BR_complete %>%
  ...
  labs(title = "Temperature over Time at Garinger High School", 
...
BR_complete %>%
  ...
  geom_line(color = "blue") +
  geom_smooth(method=lm, color= "red") +
  labs(title = "Rainfall over Time at Garinger High School", 
       x = "Date", y = "Rainfall")
...
BR_Rain_decomposed <- stl(BR_rain_ts,
                          s.window="periodic")
plot(BR_Rain_decomposed)

BR_temp_decomposed <- stl(BR_temp_ts,
                          s.window="periodic")
...
BR_Temp_Components <- as.data.frame(BR_Rain_decomposed$time.series) %>%
