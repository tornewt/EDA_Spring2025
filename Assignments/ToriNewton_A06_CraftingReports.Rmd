...
#Table 1: mean PM2.5 concentrations for each county
summary_PM25 <- summary_PM25 %>%
  mutate(`mean_PM2.5` = format(round(`mean_PM2.5`, 2)))

knitr::kable(summary_PM25, 
             caption = "Mean Particulates (2.5mm)", 
             col.names = c("County", "$\\mu g/m^3$"))

#Table 2:mean ozone concentrations for each county
summary_Ozone <- summary_Ozone %>%
  mutate(`mean_Ozone` = format(round(`mean_Ozone`, 2)))
...
```{r echo=FALSE, fig.align="left", fig.cap="Boxplot of Ozone Levels by Month(2018)", fig.height= 3.5, warning=FALSE}
#Filter the dataset for 2018
data_2018 <- epa_data %>%
...
```{r echo=FALSE, fig.align= "left", fig.cap= TRUE, fig.cap = "Boxplot of Ozone Levels by Month(2019)", fig.height= 3.5, warning=FALSE}
#Filter the dataset for 2019
data_2019 <- epa_data %>%
...
Add a table of contents and list of figures to your document and knit to a PDF. Submit your PDF to Canvas, but also be sure to commit and push your Rmd file used to create this knit document to GitHub. In the section below, add a link to your GitHub repository.

## Git Repository

https://github.com/tornewt/EDA_Spring2025.git
